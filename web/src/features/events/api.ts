...
export async function fetchEvent(eventId: string): Promise<Event | null> {
  try {
    const result = await callEdgeFunction<{ event: DatabaseEvent }>('events', `/${eventId}`, {
      method: 'GET',
      requireAuth: false,
    });
    ...
    return {
      ...
      price: eventData.ticket_price ?? eventData.price ?? 0,
 ...
 */
export async function createEvent(event: Omit<Event, 'id' | 'attendees'>): Promise<Event> {
  try {
    console.log('📝 Creating event via Edge Function...');
    
    const result = await callEdgeFunction<{ event: DatabaseEvent }>('admin-operations', '/events', {
      method: 'POST',
      body: {
        name: event.name,
    ...
    return {
      ...
      price: data.ticket_price ?? data.price ?? 0,
...
export async function updateEvent(
  ...
  try {
    ...
    const result = await callEdgeFunction<{ event: DatabaseEvent }>('admin-operations', `/events/${eventId}`, {
      method: 'PUT',
      body,
    });
