...
export const STRIPE_PUBLISHABLE_KEY = env.stripe.publishableKey;
...
export async function purchaseTicketWithStripe(
  ...
  try {
    ...
    if (ticketError) {
      ...
      if (ticketError.message?.includes('AbortError') || ticketError.message?.includes('aborted')) {
    ...
    try {
      ...
      if (errorMessage.includes('AbortError') || errorMessage.includes('aborted')) {
        ...
        console.warn('⚠️ Could not record payment (table may not exist):', error);
      }
      // Don't throw - ticket was created successfully
    }
...
export async function purchaseTicketDemo(eventId: string, amount: number) {
  try {
    ...
    if (checkError) {
      ...
      if (checkError.message?.includes('AbortError') || checkError.message?.includes('aborted')) {
    ...
    if (ticketError) {
      ...
      if (ticketError.code === '23505') { // Postgres unique violation code
        throw new Error('You already have a ticket for this event');
      }
