...
interface RouteHandler {
  pattern: RegExp;
  method: string;
  handler: (req: Request, userId: string, match: RegExpMatchArray, corsHeaders: HeadersInit) => Promise<Response>;
}
    ...
    const routes: RouteHandler[] = [
      {
        ...
        handler: async (req, _userId, _, corsHeaders) => {
          ...
          console.log(`💳 Creating payment intent: event=${eventId}, amount=$${amount}`);
          ...
          if (!stripe) {
            ...
            return serverError(
              new Error('Payment processing is not configured. Stripe secret key is missing.'),
              corsHeaders
            );
          }
      ...
      },
      {
        ...
        handler: async (req, userId, _, corsHeaders) => {
          ...
          console.log(`🎫 Processing purchase: event=${eventId}, demo=${!!isDemoMode}`);
      ...
      },
      {
        ...
        handler: async (_req, userId, match, corsHeaders) => {
          ...
          if (fetchError || !ticket) {
            return notFound('Ticket not found or already cancelled', corsHeaders);
          }
          ...
          if (!isPastEvent && ticket.stripe_payment_intent_id && !ticket.stripe_payment_intent_id.startsWith('demo_') && stripe) {
            try {
      ...
      },
      {
        ...
        handler: async (_req, userId, _, corsHeaders) => {
          ...
          const { data, error } = await supabaseAdmin
            .from('tickets')
            .select('id, event_id, payment_amount, status, purchased_at, events(id, name, date, duration, ticket_price, image_url)')
            .eq('user_id', userId)
            .eq('status', 'active')
